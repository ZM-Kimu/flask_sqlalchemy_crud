"""Internal runtime helpers for sync CRUD context management."""
...
class EnterScopeResult:
    ...
    nested_txn: SessionTransaction | None
    owns_provider_session: bool


def enter_crud_scope(
    *,
    session: SessionLike,
    existing_txn_policy: ExistingTxnPolicy,
) -> EnterScopeResult:
    """Join or create a sync transaction scope for a CRUD context."""
    ...
    nested_txn: SessionTransaction | None = None
...
def exit_crud_scope(
    *,
    ...
    close_session: Callable[[SessionLike], None],
) -> None:
    """Finalize a sync CRUD context and close owned session if needed."""
    ...
    try:
        ...
        if should_close_owned_session and session_to_close is not None:
            close_session(session_to_close)
